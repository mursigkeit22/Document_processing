function RawMTvsReference() {
  ...
  var columnReference = activeSheet.getRange('D:D');
  var valuesReference = columnReference.getValues();
  // var a = 206

  // var columnReference = activeSheet.getRange(`D7:D${a}`);
  var columnReference = activeSheet.getRange('D7:D');
  var valuesReference = columnReference.getValues();
  for (var row in valuesReference) {
    if (valuesReference[row]=== ""){
      break;
    }

      Logger.log(valuesReference[row]);
  }
  // Logger.log("valuesReference: "+valuesReference)
